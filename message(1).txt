...
function setTag(tag) {
  ...
  if (newTag.trim() == "") {
    for (var j = savedTags.length - 1; j > 0; j--) {
      if (savedTags[j][0] == setTagUsername) {
        savedTags.splice(j, 1);
      }
    }
  } else {
    var foundName = false;
    for (var k = 0; k < savedTags.length; k++) {
      if (savedTags[k][0] == setTagUsername) {
        foundName = true;
        savedTags[k][1] = newTag;
        break;
      }
    }
    if (foundName != true) {
